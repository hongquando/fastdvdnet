...
def test_fastdvdnet(**args):
    ...
    seqs_dirs = sorted(glob.glob(os.path.join(args['test_path'], "*.mat")))

    if not os.path.exists(args['save_path']):
        os.makedirs(args['save_path'])

    for i in range(len(seqs_dirs)):
        logger.info("Start denoising image{}".format(seqs_dirs[i]))
        with torch.no_grad():
        ...
        logger.info("Finished denoising {}".format(seqs_dirs[i]))
        ...
        # Save input
        input = seqn.cpu().numpy()
        input = np.reshape(input, (input.shape[0], input.shape[2], input.shape[3], 3))
        mdic = {"label": "Input of FastDVDNet", "output": input[:,:,:,0]}
        scipy.io.savemat(os.path.join(args['save_path'], "noised_image_"+str(i)+".mat"), mdic)

        # Save output
        denframes = denframes.cpu().numpy()
        denframes = np.reshape(denframes, (denframes.shape[0], denframes.shape[2], denframes.shape[3], 3))
        mdic = {"label": "Output of FastDVDNet", "output": denframes[:,:,:,0]}
        scipy.io.savemat(os.path.join(args['save_path'], "denoised_image_"+str(i)+".mat"), mdic)
